...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/26 10:44:49 by eozdur            #+#    #+#             */
/*   Updated: 2023/11/26 10:44:51 by eozdur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	other_out_filesme(t_parse *main_parse)
{
	char	str[256];
	char	*pwd;
	char	*pwd1;
	...
	if (nparse->type == 4)
		main_parse->fd = open(pwd1, O_CREAT | O_RDWR | O_APPEND, 0777);
	else if (main_parse->type == 3)
		main_parse->fd = open(pwd1, O_CREAT | O_RDWR | O_TRUNC, 0777);
	main_parse->outfile = main_parse->fd;
	if (pwd)
		free(pwd);
	if (pwd1)
		free(pwd1);
...
int	create_files_m(t_shell *main_shell)
{
	...
	while (parse)
	{
		if (parse->type == 3 || parse->type == 4)
			create_out_files_me(parse, main_shell->main_parse);
